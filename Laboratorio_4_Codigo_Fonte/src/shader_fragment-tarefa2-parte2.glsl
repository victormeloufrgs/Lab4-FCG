...
void main()
{
    ...
    //vec4 r = -l_spotlight+2*n*dot(n,l_spotlight);
    ...
    if (dot(light_dir_test, v_spotlight_test) >= cos(a_spotlight)) {
        ...
        vec4 r_spotlight = -v_spotlight+2*n*dot(n,v_spotlight);

        color += Kd*I*max(0,dot(n,l_spotlight))+ Ka*Ia + Ks*I*max(0,pow(dot(r_spotlight,v),q));
    }
